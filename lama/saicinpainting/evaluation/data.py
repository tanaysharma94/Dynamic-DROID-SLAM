...
class CustomInpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None):
        self.datadir = datadir
        # print(self.datadir)
        self.file_names = sorted(os.listdir(os.path.join(self.datadir, 'masks')))
        self.mask_filenames = [os.path.join(self.datadir,'masks', fn) for fn in self.file_names]
        self.img_filenames = [os.path.join(self.datadir,'rgb', fn) for fn in self.file_names]
        # self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, 'masks','*.png'),recursive=True)))
        # print(self.mask_filenames[:10])
        # self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*mask*.png'), recursive=True)))
        # self.img_filenames = [fname.rsplit('_mask', 1)[0] + img_suffix for fname in self.mask_filenames]
        self.pad_out_to_modulo = pad_out_to_modulo
        self.scale_factor = scale_factor
        # print("here", self.scale_factor, self.pad_out_to_modulo)
